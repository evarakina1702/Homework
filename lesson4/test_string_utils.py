...
@pytest.mark.parametrize("string, result", [(" Skypro", "Skypro"), ("  Skypro", "Skypro"), ("   Skypro", "Skypro")])
def test_trim_positive(string, result):
    string_utils = StringUtils()
    res = string_utils.trim(string)
    assert res == result

@pytest.mark.parametrize("string, result", [("", ""), ("  ", ""), ("  123", "123")])
...
@pytest.mark.parametrize("string, delimeter, result", [("a,b,c,d", ",", ["a", "b", "c", "d"]),("1:2:3:4", ":", ["1", "2", "3", "4"]), ("Привет Skypro", ",", ["Привет", "Skypro"])])
...
@pytest.mark.parametrize("string, symbol, result",[("Skypro", "S", True), ("Skypro", "U", False)])
...
@pytest.mark.parametrize("string, symbol, result",[("Skypro", "U", True), ("Skypro", "S", True)])
...
@pytest.mark.parametrize("string, symbol, result",[("SkyPro", "a", "SkyPro")])
...
@pytest.mark.parametrize("string, symbol,result", [("SkyPro", "S", True), ("SkyPro", "Y", False)])
def test_starts_with_positive(string, symbol, result):
    string_utils = StringUtils()
    res = string_utils.starts_with(string, symbol)
    assert res == result

@pytest.mark.parametrize("string, symbol,result", [("SkyPro", "A", False), ("SkyPro", "B", False)])
...
@pytest.mark.parametrize("string, symbol, result", [("SkyPro", "o", True), ("Skypro", "r", False)])
def test_end_with_positive(string, symbol, result):
    string_utils = StringUtils()
    res = string_utils.end_with(string, symbol)
    assert res == result

@pytest.mark.parametrize("string, symbol, result", [("SkyPro", "P", False), ("Skypro", "x", False)])
...
@pytest.mark.parametrize("string, result", [("", True), ("Skypro", False)])
...
@pytest.mark.parametrize("string, result", [("Skypro", True), ("", False), ("  ", False)])
...
@pytest.mark.parametrize("lst, joiner, result", [([], ", ", ""),([1], ", ", "1"),([1, 2, 3], "- ", "1-2-3"),(["Sky", "Pro"], "1", "Sky1Pro")])
